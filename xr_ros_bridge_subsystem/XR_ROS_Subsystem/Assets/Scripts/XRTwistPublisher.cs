...
public class XRTwistPublisher : MonoBehaviour
{
    ...
    public float linearScale  = 0.1f;
    [Tooltip("Multiplies angular velocity sent to Servo")]
    public float angularScale = 0.1f;

    [Header("Deadband — suppresses noise / hand tremor")]
    public float linearDeadband  = 0.005f;   // metres/frame below which we zero out
    public float angularDeadband = 0.01f;     // radians/frame
    ...
    void PublishXR()
    {
        ...
        Vector3 angVel = (Mathf.Abs(angleRad) < angularDeadband) ? Vector3.zero : (axis * angleRad / dt)  * angularScale;

        prevPos_ = pos;
        prevRot_ = rot;

        // replace the PublishTwist call at the bottom of PublishXR() with:
        bool moving = linVel != Vector3.zero || angVel != Vector3.zero;
    ...
    void PublishTwist(Vector3 linUnity, Vector3 angUnity)
    {
        ...
        if (inputMode == InputMode.XRHeadset){
            angROS = new Vector3(-angUnity.z, angUnity.x, -angUnity.y);
            linROS = new Vector3(linUnity.z, -linUnity.x, linUnity.y);
        }
