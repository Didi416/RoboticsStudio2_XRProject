...
class TcpServer(Node):
    ...
    def setup_executor(self):
        ...
        # Keep spinning even if a node handle becomes invalid on reconnect
        while rclpy.ok():
            try:
                executor.spin_once(timeout_sec=0.1)
            except Exception as e:
                self.logwarn("Executor spin error (ignored): {}".format(e))
...
class SysCommands:
        ...
        try:
            ...
            if message_class is None:
                self.tcp_server.logwarn(
                    "SysCommand.subscribe - Unknown message class '{}', skipping".format(message_name)
                )
                return
            ...
            self.tcp_server.logwarn(
                "SysCommand.subscribe - Skipping topic '{}': {}".format(topic, e)
            )
    ...
    def publish(self, topic, message_name, queue_size=10, latch=False):
        ...
        try:
            ...
            if message_class is None:
                self.tcp_server.logwarn(
                    "SysCommand.publish - Unknown message class '{}', skipping".format(message_name)
                )
                return
            ...
            self.tcp_server.logwarn(
                "SysCommand.publish - Skipping topic '{}': {}".format(topic, e)
            )
